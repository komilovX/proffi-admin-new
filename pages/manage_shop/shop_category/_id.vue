...
export default {
  ...
  async asyncData({store, route}){
    try {
      const {products, category} = await store.dispatch('shop/findCategoryById', route.params.id)
  ...
  mounted() {
    this.shopCategoryForm.title = this.category.title
    this.shopCategoryForm.products = JSON.parse(this.category.products).map(v => ({id: v.id, tip: v.tip}))
  },
  methods:{
    goToBack() {
      this.$router.back()
    ...
    submitForm(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          try {
            await this.$store.dispatch('shop/updateCategoryById', formData)
            this.$message.success('категория успешна обнавленa')
