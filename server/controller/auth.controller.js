...
module.exports.createUser = async (req, res) => {
  try {
    const condidate  = await Admins.findOne({where: { login: req.body.login }})
...
module.exports.loginSystem = async (req, res) => {
  try {
    const condidate = await Admins.findOne({ where: {login: req.body.login}})
    if(condidate) {
      const isPasswordCorrect = bcrypt.compareSync(req.body.password, condidate.password)
      if(isPasswordCorrect) {
        ...
        res.status(404).json({message: 'Такой логин и/или пароль не существует!'})
      }
    }
    else {
      res.status(404).json({message: 'Такой логин и/или пароль не существует!'})
